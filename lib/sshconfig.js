...
function SshConfig(options) {
  ...
  if (!options.configFile) {
    options.configFile = process.env[(process.platform === 'win32') ? 'USERPROFILE' : 'HOME'] + '/.ssh/config';
  }
 ...
 * @param {function(Error)} callback
 */
SshConfig.prototype.writeConfig = function (config, callback) {
  var self = this;
    ...
    if (err) {
      ...
      self._readConfigFile(function (err) {
        if (err) {
          ...
          var iterNewConfig = function (cnt, callback) {
            if (config.length > cnt) {
              var hostConfig = config[cnt],
              ...
              iterNewConfig(++cnt, callback);
            } else {
              callback();
            }
          };
          iterNewConfig(0, function (err) {
            if (err) {
              callback(err);
            } else {
              var completeConfig = '';
